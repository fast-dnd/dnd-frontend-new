...
const Dungeons = ({ dungeonType, canSelect }: IDungeonsProps) => {
  ...
  const dungeons = dungeonType === "recommended" ? recommendedDungeons : myDungeons;
  const [selectedDungeon, setSelectedDungeon] = useState<IDungeon | undefined>(undefined);
  ...
  return (
    <div className="flex flex-col gap-12 flex-1 overflow-y-auto pr-8">
      {dungeons.map((dungeon) => (
        <div
          key={dungeon._id}
          className={cn("flex flex-row gap-8 hover:bg-white/5", dungeon === selectedDungeon && "bg-white/5")}
        ...
        >
          ...
          <div className="flex flex-col py-4 gap-4 w-full">
            <div className="flex flex-row justify-between w-full pr-8">
              <p className="text-[22px] leading-7 font-medium tracking-[0.15em] uppercase">{dungeon.name}</p>
